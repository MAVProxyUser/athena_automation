  ...
  nav_msgs::msg::Path & plan)
{
  ...
  auto findLowestPoint = [&]() {
    auto getPointCost = [&](double x, double y) {
      unsigned int mx, my;
      if (!costmap_->worldToMap(x, y, mx, my)) return nav2_costmap_2d::LETHAL_OBSTACLE;
      return costmap_->getCost(mx, my);
    };
    double res = costmap_->getResolution();
    geometry_msgs::msg::Point p, best_p = goal.position;
    double cost = getPointCost(goal.position.x, goal.position.y);
    if (cost < nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE || 
            cost == nav2_costmap_2d::NO_INFORMATION) {
        best_p = goal.position;
    } else {
      double best_sdist = std::numeric_limits<double>::max();
      p.y = goal.position.y - tolerance;
      while (p.y <= goal.position.y + tolerance) {
        p.x = goal.position.x - tolerance;
        while (p.x <= goal.position.x + tolerance) {
          double cost = getPointCost(p.x, p.y);
          double sdist = std::hypot(p.x - goal.position.x, p.y - goal.position.y);
          if ((cost < nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE ||
               cost == nav2_costmap_2d::NO_INFORMATION)
                && sdist < best_sdist) {
            best_sdist = sdist;
    ...
    for (int i = -1; i <=1; ++i)
        for (int j = -1; j <= 1; ++j)
            ns.push_back(std::vector< int>{i, j});
    int n = 0;
    while(1) {
        p = best_p;
        if (std::hypot(p.x - goal.position.x, p.y - goal.position.y) >= tolerance) break;

        bool found_lower = false;
        auto best_vc = getPointCost(p.x, p.y);
        auto best_vd = 0.0;
        for (const auto &n : ns) {
            double x = p.x + n[0] * res;
            double y = p.y + n[1] * res;
            if (std::hypot(x - goal.position.x, y - goal.position.y) >= tolerance) continue;

            auto v = getPointCost(x, y);
            double d = std::hypot(n[0], n[1]);
            if (v < best_vc) {
                found_lower = true;
                best_vc = v; best_vd = d;
                best_p.x = x; best_p.y = y;
            } else if (v == best_vc && d < best_vd) {
                best_p.x = x; best_p.y = y;
            }
        }
        ++n;
        if (!found_lower) break;
    }
    return best_p;
  };
  auto final_p = findLowestPoint();
