...
void VioManager::imu_process(){
    ov_core::CameraData message;
    {
        std::unique_lock<std::mutex> lck(mtx_img);
        ...
        int csize = camera_queue.size();
        double timestamp_inC = propagator->get_last_timestamp() - state->_calib_dt_CAMtoIMU->value()(0);
        double timestamp_last_img = camera_queue.at(csize-1).timestamp; 
        double imu_timestamp = propagator->get_last_timestamp();

        if(csize > 0){
            double diff = abs(timestamp_last_img - imu_timestamp);
            if(diff > 0.15){
                std::cout << "WARNNING: diff too large!!" << std::endl;
                std::cout << "imu timestamp: " << std::setprecision(15) << imu_timestamp << std::endl;
                std::cout << "image timestamp: "  << std::setprecision(15) << timestamp_last_img << std::endl;
            }
                if(abs(pre_time - imu_timestamp)>0.15){
                std::cout << "WARNNING: large time gap!! " << abs(pre_time - imu_timestamp) << std::endl;
                std::cout << "imu timestamp: " << std::setprecision(15) << imu_timestamp << std::endl;
                std::cout << "image timestamp: "  << std::setprecision(15) << timestamp_last_img << std::endl;
            }
            pre_time = imu_timestamp;
        }
        message = camera_queue.at(0);
        camera_queue.pop_front();
        std::cout << "camera_queue size check: " << camera_queue.size() << std::endl;
    	if (message.timestamp >= timestamp_inC) {
	    return;
    	}
    }
...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
    ...
    std::cout << "time_track " << time_track << std::endl;
    ...
    // Call on our propagate and update function
    is_reset = do_feature_propagate_update(message);

    if(is_reset) {
        track_lost_edge_ = true;
        reset_cnt_++;
        std::cout << "we try to reset " << is_reset << std::endl;
        std::cout << "reset cnt: " << reset_cnt_ << std::endl;

        reset_time = message.timestamp;
        ...
        // Lets make a feature extractor
        trackDATABASE.reset(new FeatureDatabase());
        if(params.use_klt) {
            TrackBase* tmpPtr = new TrackKLT(params.num_pts,state->_options.max_aruco_features, params.use_multi_threading,params.fast_threshold,params.grid_x,params.grid_y,params.min_px_dist); 
            trackFEATS.reset(tmpPtr);
            trackFEATS->set_calibration(params.camera_intrinsics, params.camera_fisheye);
        } else {
            TrackBase* tmpPtr = new TrackDescriptor(params.num_pts,state->_options.max_aruco_features, params.use_multi_threading,params.fast_threshold,params.grid_x,params.grid_y,params.knn_ratio);
            trackFEATS.reset(tmpPtr);
            trackFEATS->set_calibration(params.camera_intrinsics, params.camera_fisheye);
        }
        ...
        // Initialize our aruco tag extractor
        if(params.use_aruco) {
            TrackBase* tmpPtr = new TrackAruco(state->_options.max_aruco_features, params.use_multi_threading, params.downsize_aruco);
            trackFEATS.reset(tmpPtr);
            trackARUCO->set_calibration(params.camera_intrinsics, params.camera_fisheye);
        }

        // Initialize our state propagator
        propagator.reset(new Propagator(params.imu_noises, params.gravity));
        ...
        // If we are using zero velocity updates, then create the updater
        if(params.try_zupt) {
            updaterZUPT.reset(new UpdaterZeroVelocity(params.zupt_options,params.imu_noises,params.gravity,params.zupt_max_velocity,params.zupt_noise_multiplier));
        }

        // Feature initializer for active tracks
        active_tracks_initializer.reset(new FeatureInitializer(params.featinit_options));

        state->_imu->set_value(pre_state_val);
        state->_imu->set_fej(pre_state_val);
        state->_timestamp = message_const.timestamp; 
        StateHelper::fix_4dof_gauge_freedoms(state, pre_state_val.block(0,0,4,1));
        trackFEATS->get_feature_database()->cleanup_measurements(state->_timestamp);
        if(trackARUCO != nullptr) {
            trackARUCO->get_feature_database()->cleanup_measurements(state->_timestamp);
        }
        std::cout << "state->_imu " << std::endl << state->_imu->quat() << std::endl;
...
bool VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {

    bool track_lose = false;
    //===================================================================================
    ...
    updaterMSCKF->update(state, featsup_MSCKF, res_cow);
    ...
    std::cout << "update slam number: " << feats_slam_UPDATE.size() << " and res: " << res_cow << std::endl;
    ...
    //std::cout << "state_cov: " << std::endl << state->_Cov.block(4,4,3,3) << std::endl;
    // std::cout << "state_cov1 size: " << state->_Cov.rows() << ", " << state->_Cov.cols() << std::endl;
    
    // slam quality evaluation 
    double pos_lose_threshold = 0.0008; //0.0005
    std::cout << res_cow << " " << state->_Cov(3,3) << " " << state->_Cov(4,4) << " " <<  state->_Cov(5,5) << std::endl;
    if ((res_cow <= 4 && feats_slam_UPDATE.size() < 15) || 
        (state->_Cov(4,4) > pos_lose_threshold) || 
        (state->_Cov(5,5) > pos_lose_threshold) ||
        (state->_Cov(3,3) > pos_lose_threshold)) {
        std::cout << "----- WRANNING: beyond threshold: " << std::endl; 
        if(track_bad_ && reset_time < 0 && message.timestamp - startup_time > 2){
            std::cout << "----- init lose, let's reset system -----" << std::endl;
            track_lose = true;
        }
        else if(track_bad_ && reset_time > 0 && message.timestamp - reset_time > 1){
            std::cout << "----- track lose, let's reset system -----" << std::endl;
            track_lose = true;
        }
        track_bad_ = true;
    } else{
        track_bad_ = false;
        pre_state_val.block(0,0,4,1) = state->_imu->quat();
        pre_state_val.block(4,0,3,1) = state->_imu->pos();
        pre_state_val.block(7,0,3,1) << 0, 0, 0;
    }
    

    //return track_lose; //tmpchange
...
bool VioManager::get_track_lost_egde() {
    bool signal = track_lost_edge_;
    if (track_lost_edge_)
        track_lost_edge_ = false;
    return signal;
}

bool VioManager::get_track_bad() {
    return track_bad_;
}
