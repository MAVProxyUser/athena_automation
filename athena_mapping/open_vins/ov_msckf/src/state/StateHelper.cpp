...
void StateHelper::EKFUpdate(std::shared_ptr<State> state, const std::vector<std::shared_ptr<Type>> &H_order,
    ...
    // std::cout << "H: " << H.rows() << ", " << H.cols() << ", K: " << K.rows() << ", " << K.cols() << std::endl;
    // std::cout << "M_a: " << M_a.rows() << ", " << M_a.cols() << std::endl;
    // std::cout << "S: " << S.rows() << ", " << S.cols() << std::endl;
    // if ((H.rows() == K.cols()) && (H.rows() > 8)) {
        // Eigen::MatrixXd var_KH = K * H;
        // std::cout << "var_KH: " << var_KH.block(4,4,3,3) << std::endl;
    // }
    
    // std::cout << "var_KH size: " << var_KH.rows() << ", " << var_KH.cols() << std::endl;
    // std::cout << "cov0: " << state->_Cov.block(4,4,3,3) << std::endl;
    ...
    // std::cout << "state_cov size: " << state->_Cov.rows() << ", " << state->_Cov.cols() << std::endl;
    // std::cout << "variable size: " << state->_variables.size() << std::endl;
    // if (res.rows() < 7)
        //std::cout << "--------res size: " << res.rows() << ", " << res.cols() << std::endl;
    // std::cout << "clone size: " << state->_clones_IMU.size() << std::endl;
    // std::cout << "feature size: " << state->_features_SLAM.size() << std::endl;
    // std::cout << "cov0: " << state->_Cov.block(4,4,3,3) << std::endl;
    // std::cout << "cov size: " << state->_Cov.rows() << ", " << state->_Cov.cols() << std::endl;
    //Cov -= K * M_a.transpose();
